$(document).ready(function() {
  ...
  function renderLocationLabels(data, textStatus, jqXHR){
    ...
    request.done(function(data){
      ...
      localStorage.setItem(key, json);
      window.cart = cart = JSON.parse(json);
      $('#shipping-time-label').text(data.shipping).fadeIn();
      $('#shipping-cost-label').text(formatMoney(data.shippingPrice)).fadeIn();
      $('#city-label').text(data.city).fadeIn();
      $('#permit-time-label').text(data.permitTime).fadeIn();
      $('#permit-cost-label').text(data.permitCost).fadeIn();
      $('#permit-notes-text').text(data.permitNotes).parent().parent().removeClass('fade');
