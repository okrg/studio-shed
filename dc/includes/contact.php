<div class="dc-contact">
  <div class="row">
    <div class="col-sm-3 offset-2">
      <img src="https://qikrg2ve4526wc3c3ntlhj1e-wpengine.netdna-ssl.com/wp-content/uploads/2015/04/DSC_1124-r2-1024x714.jpg" class="img-fluid" />
    </div>
    <div class="col-sm-5">
      <h4>Contact your Studio Shed Advisor</h4>
      <p>Have a question? Need help pricing or want to to chat about an idea? We can walk you through the entire process.</p>
      <div class="contact-buttons">
        <a target="_blank" href="https://www.studio-shed.com/contact-us/" class="btn btn-primary mb-3">Send a Message <i class="fas fa-arrow-right"></i></a>
      </div>
      <div class="contact-links">
        <div>Email <a href="mailto: answers@studioshed.com">answers@studioshed.com</a></div>
        <div>Phone <a href="tel:888-900-3933">(888) 900-3933</a></div>
      </div>
    </div>
  </div>
</div>