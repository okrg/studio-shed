...
class BVWPAdmin {
	...
	public function initHandler() {
		...
		if ($this->bvinfo->isActivateRedirectSet()) {
			...
			if ($apipubkey && isset($apipubkey)) {
				...
				wp_redirect($this->bvinfo->appUrl().'/plugin/activate?bvpublic=e7354883cbf31656fba78640914bc675&siteurl='.$siteurl.'&adminurl='.$adminurl);
				exit();
			}
	...
	public function getPluginLogo() {
		$brand = $this->bvinfo->getBrandInfo();
		if (is_array($brand) && array_key_exists('logo', $brand)) {
			return $brand['logo'];
		}
	...
	public function getWebPage() {
		$brand = $this->bvinfo->getBrandInfo();
		if (is_array($brand) && array_key_exists('webpage', $brand)) {
